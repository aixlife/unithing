...
// ── 키워드 추출 ───────────────────────────────────────────────────────────────
const STOPWORDS = new Set(['및', '을', '를', '이', '가', '은', '는', '에', '의', '로', '에서', '으로', '과', '와', '도', '하는', '있는', '통해', '대한', '위한', '있음', '하여', '등을', '등의', '등이', '하며', '이며', '이고', '하고', '위해', '그리고', '또한', '특히', '이를', '하였으며', '하였다', '지속적으로', '바탕으로', '통하여', '활동을', '활동의', '활동에', '활동이', '모습을', '모습이', '보였다', '보임', '나타냄', '발휘하며', '보여주었다', '보여줌', '기울여', '기반으로']);
...
export function Service5Haksaengbu() {
  ...
  async function handlePdf() {
    ...
    try {
      ...
      pdf.save(`${studentName}_학생부리포트.pdf`);
    } finally {
  ...
  return (
    <div ref={reportRef} style={{ fontFamily: FONT, display: 'flex', flexDirection: 'column', gap: 16, background: T.bg }}>

      {/* ── 헤더 ── */}
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 14, padding: '18px 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
        <div>
          ...
          <div style={{ fontSize: 'clamp(18px, 2vw, 24px)', fontWeight: 800, color: T.text, letterSpacing: '-0.03em', fontFamily: FONT }}>
            {studentName} 학생 학생부 리포트
          </div>
